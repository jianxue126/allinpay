...
public class BL2VarFieldCodec extends AbstractFieldCodec {
    public BL2VarFieldCodec(FieldAttribute attr) {
        super(attr);
        this.attr.maxlen >>= 3;
    }
    ...
    public byte[] encode(String data) {
        ...
        if (this.attr.compress) {
            field = new byte[1 + dataLength];
            len = FieldUtils._N2Bcd(String.format("%02d", new Object[]{Integer.valueOf(dataLength)}));
            ...
            len = FieldUtils.toBytes(String.format("%02d", new Object[]{Integer.valueOf(dataLength)}));
            val = FieldUtils.hexString2Bytes(data);
            System.arraycopy(len, 0, field, 0, len.length);
            System.arraycopy(val, 0, field, len.length, val.length);
            return field;
